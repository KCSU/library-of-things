...
def _create_loan(session: Session, item_id: uuid.UUID, user_id: uuid.UUID,
                 start_time: datetime | None = None,
                 created_by_id: uuid.UUID | None = None) -> uuid.UUID:
    ...
    outstanding = (session.query(Loan)
...
def end_loan(loan_id: uuid.UUID, acting_crsid: str,
             returned_at: datetime | None = None) -> bool:
    ...
    with db_session() as session:
        ...
        session.add(Audit(actor_crsid=acting_crsid, message=(
            f'Recorded return of {loan.item.title} '
            f'from {loan.user.crsid}')))
        return True
...
def request_item(item_id: uuid.UUID, user_session: dict[str, Any]) -> bool:
    ...
    with db_session() as session:
        ...
        existing = (session.query(Request)
        ...
        session.add(Request(item_id=item_id, user_id=user.id,
                            request_time=datetime.now()))
        session.add(Audit(actor_crsid=crsid,
                          message=f'Requested {item.title}'))

    return True


def refuse_request(request_id: uuid.UUID, reason: str,
                   acting_crsid: str) -> bool:
    ...
    with db_session() as session:
        ...
        session.delete(req)
        session.add(Audit(actor_crsid=acting_crsid, message=(
            f'Refused request from {borrower} for {title}: '
            f'{reason}')))
        return True
...
def get_all_requests() -> list[dict[str, Any]]:
    ...
    with db_session() as session:
        requests = (session.query(Request)
                    ...
                    .options(selectinload(Request.item),
...
def approve_request(request_id: uuid.UUID, acting_crsid: str) -> dict[str, Any]:
    ...
    with db_session() as session:
        ...
        session.add(Audit(actor_crsid=acting_crsid, message=(
            f'Approved request from {req.user.crsid} '
            f'for {item.title}')))
        session.delete(req)
...
def lend_to_user(item_id: uuid.UUID, crsid: str, admin_crsid: str,
                 start_time: datetime | None = None) -> dict[str, Any]:
    ...
    with db_session() as session:
        ...
        if not borrower.user_enabled:
            return {'success': False,
                    'error': f'{crsid} is no longer an active member'}
        ...
        session.add(Audit(actor_crsid=admin_crsid, message=(
